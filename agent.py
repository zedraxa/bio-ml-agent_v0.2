...
def _strip_redundant_prefixes(rel: str, proj: str) -> str:
    ...
    Strateji: Bilinen ML proje alt klasörlerini (src, data, results, vb.) arayarak
    gerçek proje-relatif yolu bul. Bu sayede ne kadar iç içe olursa olsun düzeltilir.
    ...
    """
    # Bilinen ML proje kök alt klasörleri ve dosyaları
    _KNOWN_ROOTS = {"src", "data", "results", "docs", "models", "notebooks", "tests", "config"}
    ...
    # Strateji 1: Bilinen bir kök klasör bulunursa, ondan önceki her şeyi kes
    for i, part in enumerate(parts):
        if part in _KNOWN_ROOTS:
            parts = parts[i:]
            cleaned = str(Path(*parts))
            if cleaned != original:
                log.info("✍️ WRITE_FILE yol düzeltildi: %s → %s", original, cleaned)
            return cleaned

    # Strateji 2: Son eleman bilinen bir dosyaysa ve başında fazla prefix varsa kes
    if parts and parts[-1] in _KNOWN_FILES:
        # Sadece dosya adını döndür (fazla prefix'leri sil)
        cleaned = parts[-1]
        if cleaned != original:
            log.info("✍️ WRITE_FILE yol düzeltildi: %s → %s", original, cleaned)
        return cleaned

    # Strateji 3: workspace/ ve proje adı prefix'lerini döngüsel olarak soy
    changed = True
    while changed:
        changed = False
...
def main():
    ...
    while True:
        ...
        for step in range(cfg.max_steps):
            ...
            if outside:
                log.warning("⚠️ Tool bloğu dışında metin vardı | tool=%s | dış_metin_uzunluk=%d", tool, len(outside))
