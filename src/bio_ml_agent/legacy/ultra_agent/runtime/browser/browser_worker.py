...
try:
    ...
    # Newer versions of playwright_stealth don't export stealth_sync
    try:
        ...
        def stealth_sync(page):
            log.warning("⚠️ playwright_stealth kullanılamıyor, stealth devre dışı")
            pass
...
class BrowserWorker:
    ...
    def _get_launch_args(self) -> list:
        """Chromium launch argümanlarını döndürür (anti-bot)."""
        args = [
    ...
    def _build_context_options(self, job_dir: Path) -> Dict[str, Any]:
        ...
        from bio_ml_agent.ultra_agent.runtime.browser.browser_policy import resolve_state_path
        import random

        # Modern User-Agent (Chrome 132+)
        user_agents = [
