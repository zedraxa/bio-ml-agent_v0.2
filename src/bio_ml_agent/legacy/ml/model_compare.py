...
class ModelComparator:
    ...
    def _train_and_evaluate(
        ...
        tracker.log_param("cv_folds", self.cv_folds)
        tracker.log_param("random_state", self.random_state)
        ...
        if cv_mean is not None:
            tracker.log_metric("cv_mean", cv_mean)
            tracker.log_metric("cv_std", cv_std)
    ...
    def plot_comparison(self, output_dir: str, prefix: str = "") -> Optional[Path]:
        ...
        try:
            ...
            tracker.log_artifact(chart_path)
            tracker.end_run()
        except Exception as e:
