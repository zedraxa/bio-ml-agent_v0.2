...
class ModelCapability(BaseModel):
    ...
    audio: bool = Field(default=False, description="Ses işleme yeteneği")
    file_upload: bool = Field(default=False, description="Doğrudan dosya (PDF vb) yükleme/okuma desteği")
    ...
    tier: int = Field(default=1, description="Zeka seviyesi (1: Düşük/Hızlı, 2: Orta, 3: Yüksek/Reasoning)")
    provider: str = Field(default="unknown")

# Model adı veya prefix -> Yetenek eşleştirmesi
MODEL_REGISTRY: Dict[str, ModelCapability] = {
    # OpenAI
    "gpt-4o": ModelCapability(vision=True, audio=True, file_upload=True, tool_use=True, context_window=128000, tier=3, provider="openai"),
    "gpt-4o-mini": ModelCapability(vision=True, audio=False, file_upload=True, tool_use=True, context_window=128000, tier=2, provider="openai"),
...
def filter_and_sort_models(required_caps: List[str], target_tier: int, available_providers: List[str]) -> List[str]:
    """İstenen yetenekleri, sağlanan zorluk seviyesini (tier) ve erişilebilir provider'ları baz alarak en uygun modelleri listeler/sıralar.
    Skorlama: Target tier'a ne kadar yakınsa o kadar iyi. Hedef tier altındaki modellere ceza puanı (penalty) uygulanır.
    Context penceresi büyük olanlar öne geçer.
    """
    results = []
